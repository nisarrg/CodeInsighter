...
public class CommonHelper {
    ...
    public static synchronized int getDiffYears(Date first, Date last) {
        ...
        boolean isMonthGreater = a.get(Calendar.MONTH) > b.get(Calendar.MONTH);
        boolean isMonthEqual = a.get(Calendar.MONTH) == b.get(Calendar.MONTH);
        boolean isDateEqual = a.get(Calendar.DATE) > b.get(Calendar.DATE);

        if (isMonthGreater || (isMonthEqual && isDateEqual)) {
            diff--;
        }
        return diff;
    }
